...
public class ExceptionsHandler {
    ...
    public ExceptionsHandlerResponse methodArgumentNotValidExceptionHandler(
        ...
        return new ExceptionsHandlerResponse(map, LocalDateTime.now());
    }

    @ExceptionHandler(ResponseStatusException.class)
    public ResponseEntity<ExceptionsHandlerResponse> responseStatusExceptionHandler(ResponseStatusException ex) {
        logger.error("responseStatusExceptionHandler: " + ex.getMessage());

        var map = new HashMap<String, List<String>>();
        map.put("mensagem", List.of(Objects.requireNonNull(ex.getReason())));

        var response = new ExceptionsHandlerResponse(map, LocalDateTime.now());
